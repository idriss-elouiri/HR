...
export const getDashboardData = async (req, res, next) => {
    try {
        ...
        const employeesChange = lastMonthEmployeesCount > 0 ?
            Math.round(((employeesCount - lastMonthEmployeesCount) / lastMonthEmployeesCount) * 100) : 0;
        ...
        const absencesChange = yesterdayAbsences > 0 ?
            Math.round(((todayAbsences - yesterdayAbsences) / yesterdayAbsences) * 100) : 0;
        ...
        const leavesChange = lastMonthApprovedLeaves > 0 ?
            Math.round(((approvedLeaves - lastMonthApprovedLeaves) / lastMonthApprovedLeaves) * 100) : 0;
        ...
        const lastMonthTotal = lastMonthSalaries.length > 0 ? lastMonthSalaries[0].total : 0;
        const salariesChange = lastMonthTotal > 0 ?
            Math.round(((monthlySalaries - lastMonthTotal) / lastMonthTotal) * 100) : 0;
        ...
        const activeEmployees = await Employee.aggregate([
            ...
            },
            {
                $lookup: {
                    ...
                    as: "absences",
                    pipeline: [
                        { $match: { date: today, status: "موافق عليها" } }
                    ]
                }
            },
            {
