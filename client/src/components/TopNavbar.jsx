...
const TopNavbar = () => {
  ...
  useEffect(() => {
    const mockNotifications = [
      ...
      { id: 2, title: "تقرير الأداء الشهري جاهز", time: "منذ ساعة", read: true },
      { id: 3, title: "اجتماع فريق الموارد البشرية", time: "منذ يومين", read: true }
    ];
  ...
  return (
    <header className="flex items-center justify-between p-4 bg-gradient-to-r from-indigo-600 to-indigo-700 dark:from-gray-800 dark:to-gray-900 shadow-lg text-white">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* زر وضع الليل */}
        <Tooltip content={darkMode ? "الوضع النهاري" : "الوضع الليلي"} placement="bottom">
          <button
                ...
                <div className="max-h-80 overflow-y-auto">
                  {notifications.length > 0 ? (
                    notifications.map(notification => (
                      <div
                        key={notification.id}
                        className={`p-4 border-b border-gray-100 dark:border-gray-700 transition ${notification.read
                    ...
                    <div className="p-8 text-center">
                      ...
                      <p className="text-gray-500 dark:text-gray-400 mt-2">لا توجد إشعارات</p>
                    </div>
                  )}
                </div>

                <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-b-xl">
                  <button className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline w-full text-center">
                    عرض جميع الإشعارات
                  </button>
                </div>
              </motion.div>
        ...
        <div className="relative">
          <button
            onClick={() => setIsDropdownOpen(isDropdownOpen === 'profile' ? null : 'profile')}
        ...
        {isVisible && (
          ...
          >
            <div className="bg-gray-800 text-white text-xs py-1 px-2 rounded-md whitespace-nowrap">
              {content}
              <div className="absolute w-2 h-2 bg-gray-800 rotate-45 -translate-x-1/2 -translate-y-1/2"
                style={{
                  left: "50%",
