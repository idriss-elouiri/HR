...
const SystemReports = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      const empParams = {};
      if (filters.employeeId) empParams.employeeId = filters.employeeId;
      if (filters.status) empParams.status = filters.status;
      const empQuery = new URLSearchParams(empParams).toString();
      ...
      const active = empData.data.filter(
        (e) => e.employmentStatus === "نشط"
      ).length;
      const suspended = empData.data.filter(
        (e) => e.employmentStatus === "موقوف"
      ).length;
      const terminated = empData.data.filter(
        (e) => e.employmentStatus === "مفصول"
      ).length;
      ...
      if (filters.status) salParams.status = filters.status;
      const salQuery = new URLSearchParams(salParams).toString();
      ...
      categories.forEach((key) => {
        ...
        avgSeries.push(
          Math.round(monthlySalaries[key].total / monthlySalaries[key].count)
        );
      });
      ...
      // استخدم "employee" بدلاً من "employeeId"
      if (filters.employeeId) {
        absParams.employee = filters.employeeId;
      }

      const absQuery = new URLSearchParams(absParams).toString();
      ...
      const monthlyRes = await fetch(
        `${apiUrl}/api/reports/monthly?${monthlyQuery}`,
        {
          credentials: "include",
        }
      );
      ...
      const annualRes = await fetch(
        `${apiUrl}/api/reports/annual/${filters.year}`,
        {
          credentials: "include",
        }
      );
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span
                      className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        employee.employmentStatus === "نشط"
                          ? "bg-green-100 text-green-800"
                          : employee.employmentStatus === "موقوف"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl p-6 shadow-lg">
          <div className="text-3xl font-bold mb-2">
            {salaries.length > 0
              ? (
                  salaries.reduce((sum, salary) => sum + salary.netSalary, 0) /
                  salaries.length
                ).toLocaleString("ar-EG", { maximumFractionDigits: 0 })
              : 0}{" "}
            د.ع
          </div>
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl p-6 shadow-lg">
          <div className="text-3xl font-bold mb-2">{attendance.length}</div>
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-gradient-to-r from-rose-500 to-rose-600 text-white rounded-xl p-6 shadow-lg">
          <div className="text-3xl font-bold mb-2">{absences.length}</div>
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-6 bg-gray-50">
        <div className="mb-6">
          ...
          <p className="text-gray-600 mt-2">
            إدارة وعرض جميع التقارير المتعلقة بالموظفين والعمليات
          </p>
        </div>
