...
const ZKAttendanceSection = () => {
  ...
  const handleCheckIn = async (employeeId) => {
    try {
      ...
      const employee = demoData.find(
        (d) => d.employee._id === employeeId
      )?.employee;
          ...
          <div className="bg-orange-50 rounded-xl p-4 border border-orange-100">
            <div className="flex justify-between items-center">
              ...
              >
                ...
                {loading ? "جاري التحميل..." : "توليد تقرير"}
              </motion.button>
            </div>
          </div>
        </div>
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg mb-6 flex items-start">
              ...
              <div>
                ...
                <p className="text-yellow-700 text-sm">
                  يتم عرض بيانات تجريبية للتوضيح. قم بمزامنة جهاز البصمة لعرض
                  البيانات الحقيقية.
                </p>
              </div>
            </div>
                      ...
                      prev.filter(
                        (attendanceRecord) =>
                          attendanceRecord.employee.fullName
                            .toLowerCase()
                            .includes(searchTerm) ||
                          attendanceRecord.employee.employeeId.includes(
                            searchTerm
                          )
                      )
                    );
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {loading ? (
                    ...
                    attendance.map((attendanceRecord) => (
                      <motion.tr
                        key={`${attendanceRecord._id}-${attendanceRecord.employee.employeeId}`}
                        className="hover:bg-gray-50"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-green-600">
                          {attendanceRecord.checkIn ? (
                            ...
                            <button
                              onClick={() =>
                                handleCheckIn(attendanceRecord.employee._id)
                              }
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-blue-600">
                          {attendanceRecord.checkOut ? (
                            ...
                            <button
                              onClick={() =>
                                handleCheckOut(attendanceRecord._id)
                              }
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          {attendanceRecord.workingHours?.toFixed(2) || "0.00"}{" "}
                          ساعة
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-orange-600">
                          {attendanceRecord.delay
                            ? `${Math.round(attendanceRecord.delay)} دقيقة`
                            : "--"}
                        </td>
