...
const allowanceTypes = [
  ...
  { value: "بدل رتبة", label: "بدل رتبة" }, // تمت الإضافة
];
...
const SalaryForm = ({
  ...
  const validationSchema = Yup.object({
    ...
    year: Yup.number().required("حقل مطلوب").min(2000, "يجب أن يكون سنة صحيحة"),
    ...
    socialInsurance: Yup.object().shape({
      ...
      percentage: Yup.number()
        .min(0, "يجب أن يكون رقم موجب")
        .max(100, "لا يمكن أن تتجاوز النسبة 100%"),
    }),
    ...
    if (salary) {
      ...
      if (salary.employee) {
        const emp = employees.find(
          (e) => e._id === salary.employee._id || e._id === salary.employee
        );
  ...
  const removeAllowance = (index) => {
    if (formik.values.allowances[index].type === "بدل رتبة") {
      toast.warning("لا يمكن حذف بدل الرتبة، فهو مرتبط برتبة الموظف");
      return;
    }
  ...
  useEffect(() => {
    if (selectedEmployee) {
      ...
      // إضافة بدل الرتبة تلقائياً
      if (selectedEmployee.rank) {
        const rankAllowance = rankAllowances[selectedEmployee.rank] || 0;

        // التحقق من وجود بدل الرتبة بالفعل
        const hasRankAllowance = formik.values.allowances.some(
          (allowance) => allowance.type === "بدل رتبة"
        );

        if (rankAllowance > 0 && !hasRankAllowance) {
          formik.setFieldValue("allowances", [
            ...formik.values.allowances,
            {
              type: "بدل رتبة",
              amount: rankAllowance,
              description: `بدل رتبة (${selectedEmployee.rank})`,
              date: new Date(),
            },
          ]);
        }
  ...
  const sectionClass =
    "bg-gradient-to-br from-white to-gray-50 p-5 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300";
  const sectionHeaderClass =
    "text-lg font-semibold text-blue-700 flex items-center pb-2 border-b border-blue-100";
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white rounded-2xl p-6 mb-8 shadow-xl">
          <h1 className="text-2xl md:text-3xl font-bold text-center">
            {selectedEmployee
              ? `إدارة راتب الموظف: ${selectedEmployee.fullName}`
              : "إضافة راتب جديد"}
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className={sectionClass}>
            <h3 className={sectionHeaderClass}>
              <FaMoneyBillWave className="ml-2 text-blue-500" /> المعلومات
              الأساسية
            </h3>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const amount =
                      (formik.values.baseSalary * percentage) / 100;
                    formik.setFieldValue(
                      "socialInsurance.percentage",
                      percentage
                    );
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    const percentage =
                      (amount / formik.values.baseSalary) * 100;
                    formik.setFieldValue("socialInsurance.amount", amount);
                    formik.setFieldValue(
                      "socialInsurance.percentage",
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-green-800">
                        {allowance.amount.toFixed(2)} د.ع
                      </p>
                    </div>
                  </div>
                  <div className="mb-3">
                    ...
                    <p className="text-gray-700">
                      {allowance.description || "لا يوجد وصف"}
                    </p>
                  </div>
      ...
      <div className="mt-2 text-xs text-blue-600 flex items-center">
        <FaInfoCircle className="ml-1" />
        <span>يتم احتساب هذا البدل تلقائياً بناءً على رتبة الموظف</span>
      </div>
    )}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Select
                      ...
                      value={allowanceTypes.find(
                        (opt) => opt.value === formik.values.newAllowance.type
                      )}
                      onChange={(option) =>
                        formik.setFieldValue("newAllowance.type", option.value)
                      }
                      className="basic-single"
                      classNamePrefix="select"
                    />
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-red-800">
                        {deduction.amount.toFixed(2)} د.ع
                      </p>
                    </div>
                  </div>
                  <div className="mb-3">
                    ...
                    <div>
                      ...
                      <p
                        className={`px-2 py-1 rounded-full text-xs ${
                          deduction.status === "معلقة"
                            ? "bg-yellow-100 text-yellow-800"
                            : deduction.status === "مسددة"
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Select
                      ...
                      value={deductionTypes.find(
                        (opt) => opt.value === formik.values.newDeduction.type
                      )}
                      onChange={(option) =>
                        formik.setFieldValue("newDeduction.type", option.value)
                      }
                      className="basic-single"
                      classNamePrefix="select"
                    />
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      value={deductionStatusOptions.find(
                        (opt) => opt.value === formik.values.newDeduction.status
                      )}
                      onChange={(option) =>
                        formik.setFieldValue(
                          "newDeduction.status",
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white bg-opacity-20 p-4 rounded-xl backdrop-blur-sm">
              ...
              <p className="text-2xl font-bold text-red-300">
                -
                {(
                  formik.values.deductions.reduce(
                    (sum, d) => sum + d.amount,
                    0
                  ) + formik.values.socialInsurance.amount
                ).toFixed(2)}{" "}
                د.ع
              </p>
            </div>
